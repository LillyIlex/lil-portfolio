...
export interface ContactContentProps {
  /** Slightly smaller heading treatment when this is embedded in the modal. */
  compact?: boolean;
}

/** The actual contact copy + actions, shared between the footer section and the nav-triggered modal. */
export function ContactContent({ compact = false }: ContactContentProps) {
  return (
    <div className="text-center">
      <Eyebrow>{sections.contact.eyebrow}</Eyebrow>
      <Title className="mt-2" variant={compact ? "sm" : undefined}>
        {sections.contact.title}
      </Title>
      <Paragraph variant={compact ? "md" : "lg"} className="mx-auto mt-4 max-w-2xl">
